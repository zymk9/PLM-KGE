...
def predict_by_split():
    ...
    ########## read the files and load the examples ##########
    file_path = "/content/drive/MyDrive/SimKGC/data/selected_triples_0711/freebase_selected_triples/fb_composition.csv"
    df_ex = pd.read_csv(file_path)
    ex_list = []
    head_list = list(df_ex["tail"].astype(str))
    tail_list = list(df_ex["new_tail"].astype(str))
    ##### pad zero for WN
    # head_list = ["0"*(8-len(head_id))+head_id for head_id in head_list]
    # tail_list = ["0"*(8-len(tail_id))+tail_id for tail_id in tail_list]
    relation_list = list(df_ex["relation"])
    ex_list = [{
      "head_id": head_list[idx],
      "tail_id": tail_list[idx],
      "relation": relation_list[idx],
    } for idx in range(len(head_list))]
    predictor.predict_tensor(ex_list, "fb_composition_tail_newtail")
    ##################################################################
    predictor.predict_tail_emb()
    # The result of original bert'
    df_ent_info = pd.DataFrame(columns = ["entity_id", "entity", "entity_desc"])
    for ex in entity_dict.entity_exs:
      df_ent_info = df_ent_info.append(
        {
          "entity_id": ex.entity_id,
          "entity": ex.entity,
          "entity_desc": ex.entity_desc
        }, ignore_index=True
      )
    df_ent_info.to_csv("ent_info.csv")
